...
public class first_puzzle_turret : MonoBehaviour
{
   public Transform target; // the current target (will be private but I keep it public to debug)
    ...
    void TurnToEnemy() 
    {
        ...
        Vector3 rotation = Quaternion.Lerp(partToRotate.rotation, lookRotation, Time.deltaTime * rotationSpeed * malus).eulerAngles;
        partToRotate.rotation = Quaternion.Euler(0f, rotation.y, 0f);
    }
    ...
    public void SlowDownTurret()
    {
        ...
        freeRotationSpeed = 1;
        malus = 0.1f;
        Debug.Log("Turret goes rompiche");
    }
}
