...
public class WeirdTurret : MonoBehaviour
{
    ...
    void UpdateRpc()
    {
        ...
        if (off)
        {
            ...
            // find the new target to (the player above)
            foreach(GameObject player in players)
            {
                // if the player is the main player, the one who spawns on the high part of the map
                if (player.transform.position.z > -75)
                    forcedTarget = player.transform;
            }
